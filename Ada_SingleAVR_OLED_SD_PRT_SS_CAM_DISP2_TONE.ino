...
// v0.15bis_4 03/05/2015 Tone function added
// v0.15bis-bis-bis 30/04/2015 Again using software serial for printer(D21:RX, D22:TX), camera support with preview image on OLED
...
const byte pin = 17;                      // the pin that will be sending signals to the thermalPrinterPrinter printer (connected to printer's rx)
...
SoftwareSerial thermalPrinter(99, pin);  // 
char mes[300];
int menum=0;
...
enum {
  ...
  KW_ASNAP,KW_SNAP,KW_RESET,KW_PLAY,
...
//  TV.set_hbi_hook(keyboard.begin());
  ...
  //thermalPrinter.begin(kConsoleBaud);
  //thermalPrinter.println(sentinel);
...
static unsigned char play_quoted_string(void)
{
  int i=0;
  unsigned char delim = *txtpos;
  if(delim != '"' && delim != '\'')
