...
// bis^5 SSD1351 support
// bisbisbis Color OLED (SSD1331) support
// v0.15bis-bis-bis-bis RTC activated.
...
//////////////// Newly added for graphic OLED(LCD) SSD1331 support/////////////
#include <SPI.h>
#include <Wire.h>
#include <Adafruit_GFX.h>
...
int screenMem[336]; //the implementation of frame buffer reffering to Ben Heck's
...
//                        sprintf(str,"#RGB,%d,%d,%u", x, y, pImg[0]);
                        ...
                        color=((pImg[0]>>3)<<11|(pImg[1]>>2)<<5|pImg[2]>>3);
                        x2=x*0.8;
                        y2=y*0.8;
                        display.drawPixel(x2,y2,color);
...
static void lcdChar(byte c) {
	...
	if (c == 8) {	//Backspace?
		...
		if (cursorX > 0) {
			...
			screenMem[315 + cursorX] = 32;	//Erase it from memory
			doFrame(315 + cursorX); //Redraw screen up to that amount
	...
	if (c != 13 and c != 10 and c != 8) {	//Not a backspace or return, just a normal character
		...
		screenMem[315 + cursorX] = c;
		cursorX += 1;
		if (cursorX < 21) {
	        display.setCursor(cursorX*6,8*15);		
		display.write(c);
		//display.display();
		}
	...
	if (cursorX == 21 or c == 10) {			//Did we hit Enter or go type past the end of a visible line?
		...
		for (int xg = 21 ; xg > 0 ; xg--) {
			...
			screenMem[210 + xg] = screenMem[231 + xg];
			screenMem[231 + xg] = screenMem[252 + xg];
			screenMem[252 + xg] = screenMem[273 + xg];		
			screenMem[273 + xg] = screenMem[294 + xg];
                        screenMem[294 + xg] = screenMem[315 + xg];
...
static void doBFrame(unsigned int amount) {
...
static void doFrame(unsigned int amount) {
        int xposi,yposi,yshift;
	////display.clearDisplay();
	//lcd.noCursor();
        //display.clearDisplay();
...
static void doEraseLine() {
       int xposi,yposi,yshift;
                yshift=15;
                yposi=yshift*8;
	        display.fillRect(0,yposi,127,yposi,BLACK);
}
